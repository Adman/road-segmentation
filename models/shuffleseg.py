...
def fcn8s(feed2, feed1, scorefr):
    # upscore2s
    upscore2 = Conv2DTranspose(1, (4, 4), strides=(2, 2), padding='same', use_bias=False)(scorefr)
    score_feed1 = Conv2D(1, (1, 1), use_bias=False)(feed1)
    fuse_feed1 = Add()([score_feed1, upscore2])

    # upscore4s
    upscore4 = Conv2DTranspose(1, (4, 4), strides=(2, 2), padding='same', use_bias=False)(fuse_feed1)
    score_feed2 = Conv2D(1, (1, 1), use_bias=False)(feed2)
    fuse_feed2 = Add()([score_feed2, upscore4])

    # upscore8s
    upscore8 = Conv2DTranspose(1, (16, 16), strides=(8, 8), padding='same', use_bias=False)(fuse_feed2)

    return upscore8


def shuffleseg(input_size=(480, 640, 3), loss='binary_crossentropy'):
    inp = Input(input_size)
    
    feed2, feed1, scorefr = shufflenet(inp)
